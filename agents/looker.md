...
**Status:** scaffold — confirmed in TB's stack as the **delivery layer** for Thanx (Charissa's Thanx reports arrive from `Looker <noreply@lookermail.com>`)
**Owner contact at TB:** Charissa Costa
**Reports to:** Restaurant Accountant (Tier 2) + Customer Success Watcher (Tier 3)

## What this agent knows

Looker (Google Cloud's BI platform) acts as a **scheduled-delivery and report-querying layer** between operator data warehouses and the operator's inbox. For TB specifically: Thanx data is queried through Looker explores and pushed to Charissa as recurring email reports. The agent owns:

### Looker primitives
- **Explore** — a queryable model over a data source (Thanx, in TB's case)
...
- **Email HTML body** — what TB currently uses (Charissa's inbox)
...
- **Embed mechanics** — Looker dashboards can be embedded via signed URL; not relevant for our ingest path, but relevant if TB wants to consume our reports inside their Looker.

## What we'd pull (per Charissa's Looker delivery)
- The **Thanx loyalty scheduled reports** (the actual content) — likely: signup counts, redemption rates, repeat-rate per store, tier movement, Bamba Ultra membership
...
## Cross-references
- **Thanx** — the data source under Looker for TB today
