# Action Shift (1–5 unit owner) + later Command — brief
...
Same loop. Different **name** and seat. The first product they touch is **Action Shift**. Command is not the homepage.

### Name + seats (lock this)

| Name | Who | Seat |
|---|---|---|
| **Action Shift** | 1 store through about 5 units. The owner who has to get hooked. | **Owner seat = free.** That is the giveaway. |
| **Manager FOH** | Later. Floor / front. | Paid seat. Not free. |
| **Manager BOH** | Later. Kitchen / back. | Paid seat. Not free. |
| **Command** | Same operator, more shops (the 3–50 group desk). | Paid extra locations / extra seats. Same next-action loop, ranked. |

**Vs MarginEdge (safe to say):** MarginEdge is invoices and prime cost, usually priced per location. Action Shift names **one next action** and **night proof**. We are not their GL. One owner seat is free so the little guy is not the one who pays for onboarding. Founder intent: wipe the SaaS that charges before a next action exists. **Do not print “better than anything on the planet”** as a claim — that’s the fight, not a Verified fact.

**“Wipe out SaaS”** is the job. The method is one free owner seat that works in 60 seconds, not a manifesto on the public page.
...
| **Action Shift** | 1–5 unit owner. Free **owner** seat. | Yesterday → one next action → night proof. Left rail names what’s missing. | Live `/action-shift` on main; ChatGPT site (401); this branch `/demo/action-shift.html`. |
| **Command** | Same owner, more shops later. | Same loop, ranked across shops. Not another dashboard. Not the first name they hear. | Gated `/command-center` (307 → login). |

The forgotten 1–5 unit owner gets **Action Shift**, not a Command Center logo. Extra seats (FOH manager, BOH manager) and extra locations are paid. **The owner seat stays free and has to hook.** If that seat is heavy, we became SaaS.
...
- Not “we forgot the little guy and named their desk Command Center.”
- Not MarginEdge with a new coat of paint. They invoice. We assign the next action.
...
| Void location table | Network → shop list. Stops there. No close / statement line pane. |
...
- Network miss → shop → **statement / close line** → owner → **due** wired into gated `/command-center` (the live long-scroll still has no left rail).
...
> “SaaS wanted a six-week onboard. You drop last night’s close. Sales, promos, Void Hunter come off that file. The left side wants void reasons, then the invoice so SKU drift is a real move — switch the beef house or raise the plate. You own tonight. We don’t invent the save. We don’t mail anyone. One seat is free. That’s Never 86'd.”
...
| `src/` | Waitlist / close (schema still named `z_reports`) / invoice / briefing stubs only. **No Command routes.** |
| `public/demo/action-shift.html` | Owner desk. Asks for POS end-of-day, not “Z report.” Pack: sales, promos, Void Hunter, SKU drift. |
| `public/demo/command.html` | Redirects to Action Shift so a 1–5 owner never lands on a Command logo. |
...
- Split so we don’t mix Bamba again: `docs/TWO_TRACKS.md`
- This file: `docs/COMMAND_DRILLDOWN.md`
- 1–5 owner try-it: `public/demo/action-shift.html`
