...
export const AGENT_SPECS: AgentSpec[] = [
  ...
  },
  {
    ...
    tag: 'Delivery',
    seat: 'CFO · CEO',
    headline: 'Statement cost, payout math, and the next evidence needed.',
    intro: 'Start with one redacted DoorDash statement. We separate observed commission, fees, restaurant-funded promotions, refunds, adjustments, and expected payout without pretending a statement alone proves a contract violation.',
    catches: [
      'Eligible sales and every disclosed restaurant-borne deduction',
      'Commission separated from restaurant-funded ads and promotions',
      'Expected payout compared with the marketplace-reported payout',
      'Explicit missing-evidence list before any contract or bank claim',
    ],
    needs: 'One finalized redacted marketplace statement. Optional contract or rate card, payout record, POS/accounting export, and bank deposit close additional tests.',
  ...
  },
  {
    ...
    tag: '3P Rates',
    seat: 'CFO · CEO',
    headline: 'Contract rule versus statement charge, with both sources attached.',
    intro: 'A statement can show observed cost. A rate-card test needs the governing agreement and the correct fee base. We compare the two only when both are supplied and label the result unresolved when a required rule is missing.',
    catches: [
      'Signed or current rate rule tied to the correct store and period',
      'Eligible-sales base used by the statement calculation',
      'Observed charge compared with a reproducible expected charge',
      'Unresolved status when the contract, fee base, or statement detail is missing',
    ],
    needs: 'The governing contract or current rate card plus a finalized marketplace statement for the same store and period.',
