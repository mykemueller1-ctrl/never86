...
describe('owner desk post-auth redirect', () => {
  ...
  it('keeps the authenticated first screen as the SimpleOwnerDemo composer', () => {
    ...
    expect(phone).toContain('DAY1_WEIRD_ASK');
    ...
    expect(phone).toContain('DAY1_SEAT_LINE');
    expect(phone).toContain('DAY1_NO_RIP_LINE');
    ...
    expect(phone).not.toMatch(/Got a truck ticket or invoice\? Snap it/);
