...
export async function getOperatorReport(
  ...
  // Net sales come from v_first_party_digital — the de-duplicated, leaf-level
  // channel view — NOT toast_location_breakdown, which double-counts. This is
  // the same source the command center uses, so both surfaces reconcile.
  const periodRows = await sql<{ period_start: string | null; period_end: string | null }[]>`
    select min(period_start)::text as period_start, max(period_end)::text as period_end
    from v_first_party_digital
    where operator_id = ${operatorId}`;
  const periodEnd = periodRows[0]?.period_end ?? '';
  const periodStart = periodRows[0]?.period_start ?? '';
  ...
  const locationRows = await sql<
    { name: string; city: string; state: string; net_sales: number }[]
  >`
    select l.name,
           l.city,
           l.state,
           round(sum(v.all_channels_net))::float8 as net_sales
