...
const topics: { id: DemoTopic; title: string; short: string; icon: 'receipt' | 'clock' | 'glass'; detail: string }[] = [
  { id: 'invoices', title: 'Vendors & invoices', short: 'Vendors', icon: 'receipt', detail: 'Same product. The whole order.' },
...
export function OperatorDemo() {
  ...
  return <div className={s.app}>
    ...
    <aside className={s.sidebar}>
      ...
      <div className={s.sidebarFooter}><span className={s.avatar}>Y</span><div><strong>Your sample seat</strong><span>First look · no signup</span></div><span className={s.freePill}>DEMO</span></div>
    </aside>
    <div className={s.mainWrap}>
      ...
      <main id="demo-main" className={s.layout}>
        <section className={s.mainColumn}>
          <div className={s.intro}><p className={s.eyebrow}>A LITTLE LESS CHAOS. A CLEAR NEXT MOVE.</p><h1>{topic === 'invoices' ? <>The price moved.<br /><em>Let’s find out why.</em></> : <>See what changed.<br /><em>Know what to do.</em></>}</h1><p>{topic === 'invoices' ? 'You have a kitchen to run. Start with the numbers, ask the right question, and keep the reply.' : 'Busy shift? Start with one thing. Try a calculation using the sample records below.'}</p></div>
        ...
        <aside className={s.context} aria-label="How the operator desk works"><div className={s.contextPaper}><span className={s.contextPin} aria-hidden="true">↗</span><p className={s.eyebrow}>MAKE IT A HABIT</p><h2>One question.<br /><em>One person.<br />One answer back.</em></h2><ol><li><span>1</span><div><strong>Check the records</strong><p>Show the numbers and where they came from.</p></div></li><li><span>2</span><div><strong>Give one person the job</strong><p>A name. A due time. A clear question.</p></div></li><li><span>3</span><div><strong>Bring back the proof</strong><p>The reply, photo or corrected record.</p></div></li></ol><div className={s.receiptStamp}>KEEP THE RECEIPT<Icon name="check" size={14} /></div></div><div className={s.tryNext}><p className={s.eyebrow}>ALSO ON THE DESK</p>{topics.filter(item => item.id !== topic).map(item => <button type="button" key={item.id} onClick={() => selectTopic(item.id)}><Icon name={item.icon} size={18} /><span>{item.title}<small>{item.detail}</small></span><Icon name="arrow" size={15} /></button>)}</div><p className={s.contextNote}>{completeChecks ? `${completeChecks} of 3 sample checks tried.` : 'Start with the invoices.'}<br />One useful result is enough to begin.</p></aside>
      </main>
    </div>
    {draftOpen && <dialog className={s.dialog} ref={draftDialog} onCancel={() => setDraftOpen(false)} onClick={event => { if (event.target === event.currentTarget) { draftDialog.current?.close(); setDraftOpen(false); } }} aria-labelledby="demo-draft-title"><div className={s.dialogHeading}><div><p className={s.eyebrow}>ONE PERSON. ONE NEXT STEP.</p><h2 id="demo-draft-title">Make the next move clear.</h2></div><button type="button" aria-label="Close draft" onClick={() => { draftDialog.current?.close(); setDraftOpen(false); }}><Icon name="close" /></button></div><form onSubmit={keepDraft}><p className={s.dialogIntro}>This is a sample handoff. It stays in this tab and is never sent.</p>{topic === 'invoices' && <label className={s.questionLabel}>Your question<textarea rows={8} value={question} onChange={event => { setQuestion(event.target.value); setDraftKept(false); }} /></label>}<div className={s.formFields}><label>Who owns the follow up?<select aria-label="Who owns the follow up?" value={owner} onChange={event => { setOwner(event.target.value as DemoDraft['owner']); setDraftKept(false); }}><option>Alex · sample manager</option><option>Riley · sample owner</option></select></label><div className={s.fieldPair}><label>When is it due?<select aria-label="When is it due?" value={dueDay} onChange={event => { setDueDay(event.target.value); setDraftKept(false); }}><option>Next shift</option><option>Tomorrow</option><option>Next delivery</option></select></label><label>At what time?<input required type="time" value={dueTime} onChange={event => { setDueTime(event.target.value); setDraftKept(false); }} /></label></div></div><div className={s.proofBox}><Icon name="receipt" size={19} /><div><strong>What counts as proof?</strong><p>{topic === 'invoices' ? `${vendorHandoff?.proof ?? 'Vendor reply and confirmed order terms'}.` : topic === 'labor' ? 'Manager note explaining the hours, with the time record.' : 'Confirmed recipe and measured pour.'}</p></div></div>{draftKept ? <div role="status" className={s.draftSuccess}><Icon name="check" /><p>Draft kept in this tab. Nothing was sent.</p><button type="button" className={s.primary} onClick={() => { draftDialog.current?.close(); setDraftOpen(false); }}>Back to the desk<Icon name="arrow" size={16} /></button></div> : <button type="submit" className={s.primary}>Keep this demo draft<Icon name="check" size={17} /></button>}</form></dialog>}
  </div>;
}
