...
export function OperatorDemo() {
  ...
  const contextSteps = topic === 'invoices' ? [
    ['Catch the price change', 'Both vendors. Last week and this week.'],
    ['See the menu impact', 'Your recipe cost against your target.'],
    ['Review the next step', 'The question, person and due time are ready.'],
  ] : topic === 'labor' ? [
    ['Compare the hours', 'The schedule beside the time clock.'],
    ['Check why it ran late', 'Extra prep, a late close or a missed punch.'],
    ['Review the follow up', 'Ask for the reason before the next shift.'],
  ] : [
    ['Start with the bottle', 'What you paid and how much is in it.'],
    ['Check your house pour', 'See the spirit cost at your standard pour.'],
    ['Confirm the recipe', 'Add mixers and garnish for the full drink cost.'],
  ];
  ...
  return <div className={s.app}>
    ...
    <aside className={s.sidebar}>
      ...
      <div className={s.sidebarFooter}><span className={s.avatar}>Y</span><div><strong>Your owner seat</strong><span>1 location · 1 seat free</span></div><span className={s.freePill}>DEMO</span></div>
    </aside>
    <div className={s.mainWrap}>
      ...
      <main id="demo-main" className={s.layout}>
        <section className={s.mainColumn}>
          <div className={s.intro}><p className={s.eyebrow}>A LITTLE LESS CHAOS. A CLEAR NEXT MOVE.</p><h1>{topic === 'invoices' ? <>Know what went up.<br /><em>Before you order again.</em></> : <>See what changed.<br /><em>Know what to do.</em></>}</h1><p>{topic === 'invoices' ? 'Start with this week’s invoice and last week’s. See which items went up, then check your other vendor.' : 'Busy shift? Start with one thing. Check the hours or what goes in the glass.'}</p></div>
          ...
          {!wasChecked && <div className={s.checkRow}><button type="button" className={s.primary} disabled={!ready} onClick={runCheck}>{topic === 'labor' ? 'Compare the hours' : 'Calculate this pour'}<Icon name="arrow" size={18} /></button><span>No signup. Records ready.</span></div>}</>}
        ...
        <aside className={s.context} aria-label="How your owner seat works"><div className={s.contextPaper}><span className={s.contextPin} aria-hidden="true">↗</span><p className={s.eyebrow}>MAKE IT A HABIT</p><h2>You run the place.<br /><em>We line up<br />the next step.</em></h2><ol>{contextSteps.map(([title, detail], index) => <li key={title}><span>{index + 1}</span><div><strong>{title}</strong><p>{detail}</p></div></li>)}</ol><div className={s.receiptStamp}>KEEP THE RECEIPT<Icon name="check" size={14} /></div></div><div className={s.tryNext}><p className={s.eyebrow}>ALSO IN YOUR SEAT</p>{topics.filter(item => item.id !== topic).map(item => <button type="button" key={item.id} onClick={() => selectTopic(item.id)}><Icon name={item.icon} size={18} /><span>{item.title}<small>{item.detail}</small></span><Icon name="arrow" size={15} /></button>)}</div><p className={s.contextNote}>{completeChecks ? `${completeChecks} of 3 sample checks tried.` : 'Start with the invoices.'}<br />One useful result is enough to begin.</p></aside>
      </main>
    </div>
