...
export default function Home() {
  ...
  return (
    <main className="compass min-h-screen">
      ...
      {/* Public 3P evidence center · descriptive internal links for operators and crawlers */}
      <section data-reveal className="border-t border-[#e8e8ed] py-20 md:py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            <div>
              <p className="compass-eyebrow mb-4">— Public 3P evidence center</p>
              <h2 className="compass-display text-4xl md:text-6xl mb-5">
                Bring the statement. <em>Follow the money path.</em>
              </h2>
              <p className="compass-body text-lg max-w-xl">
                Fifty-two source-linked field guides for restaurant operators reviewing DoorDash, Uber Eats, Grubhub, and ezCater statements, payouts, promotions, refunds, and bank deposits.
              </p>
              <div className="mt-7 flex flex-wrap gap-3">
                <Link href="/delivery-marketplace-reconciliation" className="btn-primary" style={{ background: '#0066ff' }}>Open the 3P evidence center →</Link>
                <Link href="/research/3p-operator-signal-august-2026" className="btn-secondary">Read the public signal research</Link>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {TOP_THREE_P_GUIDES.map((guide, index) => (
                <Link key={guide.href} href={guide.href} className="compass-card hover:border-[#0066ff] transition-colors block group">
                  <p className="font-mono text-[11px]" style={{ color: '#0066ff' }}>{String(index + 1).padStart(2, '0')}</p>
                  <h3 className="!mt-3 text-lg">{guide.title}</h3>
                  <p className="compass-body text-sm mt-3 group-hover:underline" style={{ color: '#0066ff' }}>Read the evidence guide →</p>
                </Link>
              ))}
            </div>
          </div>
