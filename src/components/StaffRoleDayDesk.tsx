...
export function StaffRoleDayDesk({
  ...
  const [weekday, setWeekday] = useState<(typeof STAFF_ROLE_DAY_WEEKDAYS)[number]>(initialWeekday);
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      <p className="text-xs uppercase tracking-[0.16em] text-amber-200">
        Role-day desk · {STAFF_ROLE_DAY_PACK_STATUS} · wall templates
      </p>
      <h2 className="mt-2 font-semibold">{desk.stationLabel} · {desk.weekday}</h2>
      <p className="mt-2 text-sm text-white/60">
        Today&apos;s checklist from the wall pack, plus the policies this seat needs.
        Cooked food is a promo, not a void. Cost bands are policy percents, not this week&apos;s dollars.
        No live credentials. No recipe book.
      </p>
