...
function Kpi({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: 'red' | 'gold' | 'green' }) {
  const v = tone === 'red' ? 'text-red-300' : tone === 'green' ? 'text-green-300' : 'text-ink-800';
  return (
...
function SampleBanner() {
  return (
    <div className="bg-ink-100 border border-ink-200 rounded-xl p-4 mb-8 flex items-center justify-between gap-4">
      ...
      <TrackedLink href="/operators#talk" event="demo_connect_data_click" meta={{ agent: 'Labor Leak', target: '/operators#talk' }} className="shrink-0 bg-ink-800 hover:bg-ink-900 text-ink-800 font-semibold rounded-full px-4 py-2 text-sm">
        Connect your data
      </TrackedLink>
    </div>
  );
...
export function LaborLeakBody({ data: d, sample }: { data: LaborLeak; sample?: boolean }) {
  ...
  return (
    <>
      ...
      <div className="mb-8">
        <h3 className="text-ink-800 text-xs uppercase tracking-wider font-semibold mb-3">Stores above budget</h3>
                  ...
                  <tr key={s.name} className="border-b border-ink-200/60 last:border-0">
                    ...
                    <td className="px-4 py-2 text-right">
                      <span className="flex items-center justify-end gap-3">
                        ...
                        <span className="tabular-nums text-ink-800">{pct(s.laborPct)}</span>
      ...
      <div className="mb-8">
        <h3 className="text-ink-800 text-xs uppercase tracking-wider font-semibold mb-3">Top schedule-vs-clocked drift</h3>
        <div className="card rounded-xl overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-ink-200 text-ink-500">
