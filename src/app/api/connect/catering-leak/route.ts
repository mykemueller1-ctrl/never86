...
import { runCateringLeak } from '@/lib/cateringLeakCsv';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!csv || csv.length > MAX_CSV_BYTES) {
      return NextResponse.json({ ok: false, error: 'Send a CSV in the body.' }, { status: 400 });
    }

    const result = runCateringLeak(csv);

    logVisitorEvent({
      eventType: 'connect_agent_run',
      pagePath: '/connect/catering-leak',
      agentName: 'Catering Leak',
      audience: 'owner',
      meta: 'orders' in result ? {
        orders: result.orders, totalGap: result.totalGap,
        unmatched: result.unmatchedOrders.length, flagged: result.flaggedOrders.length,
      } : { error: result.error },
    }).catch(() => {});
