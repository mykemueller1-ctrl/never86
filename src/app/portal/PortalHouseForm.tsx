...
import { ORCHESTRATION_BRAND_BLUE } from '@/lib/orchestration';
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <label className="block">
        ...
        <input
          ...
          style={{ boxShadow: `0 0 0 1px ${code ? ORCHESTRATION_BRAND_BLUE : 'transparent'}` }}
        />
      </label>
      <button type="submit" className="btn-primary w-full" style={{ background: ORCHESTRATION_BRAND_BLUE }}>
        Open the seat →
      </button>
      {message ? (
        <p className="text-sm" style={{ color: ok ? ORCHESTRATION_BRAND_BLUE : '#86868b' }}>
          {message}
        </p>
      ) : null}
    </form>
