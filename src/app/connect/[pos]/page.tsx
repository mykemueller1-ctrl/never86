    ...
    <main className="compass min-h-screen">
      ...
      <section className="max-w-5xl mx-auto px-6 pt-12 md:pt-16 pb-12">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {p.partnerApplyUrl && (
            <TrackedLink href={p.partnerApplyUrl} external event="connect_pos_partner_portal_click" meta={{ pos: p.slug, target: p.partnerApplyUrl, label: `${p.name} partner portal` }} className="btn-secondary" style={{ background: 'transparent', borderColor: '#2c2c2e', color: '#ffffff' }}>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <ol className="space-y-4 compass-body text-[15.5px] leading-relaxed">
            ...
            <li className="flex gap-4">
              ...
              <span>Drop the CSV at <TrackedLink href="/trial" event="connect_pos_inline_trial_click" meta={{ pos: p.slug, label: '/trial inline' }} className="underline text-white" style={{ textDecorationColor: '#0066ff' }}>/trial</TrackedLink>. We auto-detect the column shape — no mapping step needed.</span>
            </li>
          </ol>
        </div>
      </section>
      ...
      <section className="border-t border-[#1f1f1f] py-16 md:py-20 px-6">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <TrackedLink href="/pricing" event="connect_pos_bottom_cta_click" meta={{ pos: p.slug, target: '/pricing', label: 'See pricing', variant: 'secondary' }} className="btn-secondary" style={{ background: 'transparent', borderColor: '#2c2c2e', color: '#ffffff' }}>See pricing</TrackedLink>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-[#1f1f1f] py-10 px-6">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3 text-[#6e6e73] text-[12px]">
          ...
          <div className="flex items-center gap-5">
            <TrackedLink href="/trial"   event="connect_pos_footer_click" meta={{ pos: p.slug, target: '/trial',   label: 'Trial' }}   className="hover:text-white transition-colors">Trial</TrackedLink>
            <TrackedLink href="/agents"  event="connect_pos_footer_click" meta={{ pos: p.slug, target: '/agents',  label: 'Agents' }}  className="hover:text-white transition-colors">Agents</TrackedLink>
            <TrackedLink href="/pricing" event="connect_pos_footer_click" meta={{ pos: p.slug, target: '/pricing', label: 'Pricing' }} className="hover:text-white transition-colors">Pricing</TrackedLink>
            <TrackedLink href="/"        event="connect_pos_footer_click" meta={{ pos: p.slug, target: '/',        label: 'Home' }}    className="hover:text-white transition-colors">Home</TrackedLink>
          </div>
        </div>
      </footer>
    </main>
